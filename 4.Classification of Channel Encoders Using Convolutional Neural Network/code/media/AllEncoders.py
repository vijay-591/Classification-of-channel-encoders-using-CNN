...
def start_process():
    ...
    finalList = []
    for row in df.index:
        text = df['Review Text'][row]
        # print(text)
        binary_data = text_to_binary(text)

        # Encode using block encoder
        block_encoded_data = list(block_encode(binary_data))
        block_encoded_data = block_encoded_data[:16]
        block_encoded_data.append("Block")
        finalList.append(block_encoded_data)
        encoder_size = 16  # len(block_encoded_data)

        # Encode using convolutional encoder
        conv_encoder = ConvolutionalEncoder()
        conv_encoded_data = conv_encoder(torch.tensor(binary_data, dtype=torch.float32))
        conv_Data = list(conv_encoded_data.detach().numpy())
        conv_Data = conv_Data[:encoder_size]
        conv_Data.append("Conv")
        finalList.append(conv_Data)
        # Encode using BCH encoder
        bch_encoded_data = bch_encode(text, encoder_size)
        bch_encoded_data.append("bch")
        finalList.append(bch_encoded_data)

        # Encode using polar encoder
        polar_encoded_data = list(polar_encode(binary_data))
        polar_encoded_data = polar_encoded_data[:encoder_size]
        polar_encoded_data.append("polar")
        finalList.append(polar_encoded_data)
    df2 = pd.DataFrame(finalList)
    df2.to_csv("Rama.csv",index=False)
